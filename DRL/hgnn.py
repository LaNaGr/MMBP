...
class GATedge(nn.Module):
    '''
    Used in PPO_Model-> Get_action_prob -> get_machines, to get hidden layer of machines 'hmas'
    ...
    def forward(self, ope_ma_adj_batch, batch_idxes, feat):
        ...
        ############################################################################
        el_add_ee = ope_ma_adj_batch.unsqueeze(-1) * el.unsqueeze(-2) + ee      #[batch_idxes] for omab
        a = el_add_ee + ope_ma_adj_batch.unsqueeze(-1) * er.unsqueeze(-3)  # [batch_idxes] for omab
        ######################################################################
        ...
        #####################################################################
        mask = torch.cat((ope_ma_adj_batch.unsqueeze(-1)==1,
                          torch.full(size=(ope_ma_adj_batch.size(0), 1,
                                           ope_ma_adj_batch.size(2), 1),
                                     dtype=torch.bool, fill_value=True)), dim=-3)
        # [batch_idxes] for omab
        #######################################################################
